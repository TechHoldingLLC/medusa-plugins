...
import { CognitoAdminStrategy } from '../../admin';
import { AUTH_PROVIDER_KEY } from '../../../../types';
import { CognitoOptions, COGNITO_ADMIN_STRATEGY_NAME, ExtraParams, Profile } from '../../types';

describe('Cognito admin strategy verify callback', function () {
	...
	let cognitoAdminStrategy: CognitoAdminStrategy;
						...
						retrieveByEmail: jest.fn().mockImplementation(async (email: string) => {
							...
							if (email === existsEmailWithProviderKey) {
								return {
									id: 'test2',
									metadata: {
										[AUTH_PROVIDER_KEY]: COGNITO_ADMIN_STRATEGY_NAME,
									},
								};
							}
	...
	describe('when strict is set to admin', function () {
		beforeEach(() => {
			cognitoAdminStrategy = new CognitoAdminStrategy(
				container,
				{} as ConfigModule,
				{
					region: 'fakeRegion',
		...
		it('should succeed', async () => {
			...
			const data = await cognitoAdminStrategy.validate(req, accessToken, refreshToken, extraParams, profile);
			expect(data).toEqual(
		...
		it('should fail when a user exists without the auth provider metadata', async () => {
			...
			const err = await cognitoAdminStrategy
